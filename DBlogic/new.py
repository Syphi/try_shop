...
def init_connection_to_db(name, login, password, host):
    db_string = f'postgresql://{login}:{password}@{host}/{name}'
...
class DbSqlalQueries:
    ...
    def init_tables(self):
        ...
        for key, value in self.metadata.tables.items():
            tables[key] = sa.Table(str(key), self.metadata, autoload=True, autoload_with=self.connection)
        return tables
...
# conn = init_connection_to_db(init_dict['name'], init_dict['login'], init_dict['password'], init_dict['host'])
# DB = DbSqlalQueries(conn)
